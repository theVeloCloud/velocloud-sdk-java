...
public final class ServiceProvider implements SharedServiceProvider<Service> {
    ...
    public @NotNull List<Service> findAll() {
        return blockingStub.find(ServiceFindRequest.getDefaultInstance()).getServicesList().stream().map(Service.Companion::fromSnapshot).toList();
    }

    @Override
    @Nullable
    public Service find(@NotNull String name) {
        return blockingStub.find(ServiceFindRequest.newBuilder().setName(name).build()).getServicesList().stream().map(Service.Companion::fromSnapshot).findFirst().orElse(null);
    }
    ...
    public List<Service> findByGroup(@NotNull String group) {
        return blockingStub.find(ServiceFindRequest.newBuilder().setGroupName(group).build()).getServicesList().stream().map(Service.Companion::fromSnapshot).toList();
    }
    ...
    public List<Service> findByType(@NotNull GroupType type) {
        return blockingStub.find(ServiceFindRequest.newBuilder().setType(type).build()).getServicesList().stream().map(Service.Companion::fromSnapshot).toList();
    }
